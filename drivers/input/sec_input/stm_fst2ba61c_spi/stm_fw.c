...
static const int stm_ts_fw_updater(struct stm_ts_data *ts, const u8 *fw_data)
{
	...
	int retry;
	u16 fw_main_version;
	u16 fw_config_version, fw_code_version;
	...
	fw_main_version = (u16)header->ext_release_ver & 0xFF;
	fw_config_version = (u16)header->cfg_ver;
	fw_code_version = (u16)header->code_ver;
	input_info(true, ts->dev,
			"%s: Starting firmware update  bin : code:0x%04X, config:0x%04X, main:0x%04X\n", __func__,
			fw_code_version, fw_config_version, fw_main_version);
	...
	while (1) {
		...
		if (retval >= 0) {
			...
			if (fw_main_version == ts->plat_data->img_version_of_ic[SEC_INPUT_FW_VER] &&
				fw_config_version == ts->config_version_of_ic &&
				fw_code_version == ts->code_version_of_ic) {
				input_info(true, ts->dev,
...
int stm_ts_fw_update_on_probe(struct stm_ts_data *ts)
{
	...
	ts->code_version_of_bin = (u16)header->code_ver;
	ts->config_version_of_bin = (u16)header->cfg_ver;
	ts->plat_data->img_version_of_bin[SEC_INPUT_FW_VER] = (u8)(header->ext_release_ver & 0xFF);
	ts->plat_data->img_version_of_bin[SEC_INPUT_FW_VER_PROJECT_ID] = (u8)header->project_id;
	ts->plat_data->img_version_of_bin[SEC_INPUT_FW_IC_VER] = (u8)header->ic_name;
	ts->plat_data->img_version_of_bin[SEC_INPUT_FW_MODULE_VER] = (u8)header->module_ver;

	if (sec_input_need_fw_update(ts->plat_data))
		retval = STM_TS_NEED_FW_UPDATE;
	else if ((ts->config_version_of_ic < ts->config_version_of_bin)
			|| (ts->code_version_of_ic < ts->code_version_of_bin))
		retval = STM_TS_NEED_FW_UPDATE;
	else
		retval = STM_TS_NOT_ERROR;

	if (ts->plat_data->hw_param.checksum_result) {
		input_err(true, ts->dev, "%s: do force update (checksum error)\n", __func__);
		retval = STM_TS_NEED_FW_UPDATE;
		...
		input_err(true, ts->dev, "%s: do force update (wrong version)\n", __func__);
		retval = STM_TS_NEED_FW_UPDATE;
	}
...
static int stm_ts_load_fw(struct stm_ts_data *ts, int update_type)
{
	...
	if (!ts->plat_data->external_firmware_data || (ts->plat_data->external_firmware_size <= 0)) {
		input_err(true, ts->dev, "firmware is not available\n");
		return -ENOENT;
	}

	header = (struct stm_ts_header *)ts->plat_data->external_firmware_data;
	...
	if (is_fw_signed) {
		...
		org_size = ts->plat_data->external_firmware_size - SPU_METADATA_SIZE(TSP);
		spu_ret = spu_firmware_signature_verify("TSP",
				ts->plat_data->external_firmware_data,
	...
	input_info(true, ts->dev,
			...
			(u16)header->code_ver, (u16)header->ext_release_ver,
	...
	error = stm_ts_fw_updater(ts, ts->plat_data->external_firmware_data);
