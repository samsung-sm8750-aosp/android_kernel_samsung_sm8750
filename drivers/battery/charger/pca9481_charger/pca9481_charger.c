...
static int pca9481_send_rx_voltage(struct pca9481_charger *pca9481, unsigned int msg_type)
{
	...
	/* Set the property */
	ret = psy_do_property("wireless", set,
				POWER_SUPPLY_PROP_VOLTAGE_NOW, pro_val);
	if (ret < 0) {
...
static int pca9481_get_rx_max_power(struct pca9481_charger *pca9481)
{
	...
	ret = psy_do_property("wireless", get,
				POWER_SUPPLY_PROP_VOLTAGE_MAX, pro_val);
	if (ret < 0) {
	...
	ret = psy_do_property("wireless", get,
				POWER_SUPPLY_PROP_CURRENT_MAX, pro_val);
	if (ret < 0) {
...
#if IS_ENABLED(CONFIG_SEC_ABC)
		pca9481->dc_vbat_check_cnt = (conv_adc > pca9481->pdata->dc_batt_ov) ?
			pca9481->dc_vbat_check_cnt + 1 : 0;

		if (pca9481->dc_vbat_check_cnt >= pca9481->dc_vbat_max_cnt) {
#if IS_ENABLED(CONFIG_SEC_FACTORY)
			sec_abc_send_event("MODULE=battery@INFO=dc_batt_ov");
#else
			sec_abc_send_event("MODULE=battery@WARN=dc_batt_ov");
#endif
			pca9481->dc_vbat_check_cnt = 0;
		}
#endif
...
/* Set new input current for fpdo */
static int pca9481_set_new_iin_fpdo(struct pca9481_charger *pca9481)
{
	int ret = 0;
...
/* Set new float voltage for fpdo */
static int pca9481_set_new_vfloat_fpdo(struct pca9481_charger *pca9481)
{
	int ret = 0;
...
static int pca9481_charge_fpdo_cvmode(struct pca9481_charger *pca9481)
{
	...
	if (pca9481->req_new_iin == true) {
		ret = pca9481_set_new_iin_fpdo(pca9481);
	} else if (pca9481->req_new_vfloat == true) {
		ret = pca9481_set_new_vfloat_fpdo(pca9481);
	} else {
		cvmode = pca9481_check_dcmode_status(pca9481);
		if (cvmode < 0) {
			ret = cvmode;
...
#if IS_ENABLED(CONFIG_SEC_ABC)
	/* abc batt ov */
	ret = of_property_read_u32(np_pca9481, "pca9481,dc-batt-ov",
							   &pdata->dc_batt_ov);
	if (ret) {
		pr_info("%s: pca9481,dc-batt-ov is Empty\n", __func__);
		pdata->dc_batt_ov = 4470000;
	}
	pr_info("%s: pca9481,dc_batt_ov is %d\n", __func__, pdata->dc_batt_ov);
#endif
...
#else
static int pca9481_probe(struct i2c_client *client, const struct i2c_device_id *id)
#endif
{
